...
function Cart() {
    ...
    let cott= localStorage.getItem("sellcottData");
    let cottobj =JSON.parse(cott);
    let jut= localStorage.getItem("selljutData");
    let jutobj =JSON.parse(jut);
    let coff= localStorage.getItem("sellcoffData");
    let coffobj =JSON.parse(coff);
    let steel= localStorage.getItem("sellsteelData");
    let steelobj =JSON.parse(steel);
    let alum= localStorage.getItem("sellalumData");
    let alumobj =JSON.parse(alum);
    let copp= localStorage.getItem("sellcoppData");
    let coppobj =JSON.parse(copp);
    let woo= localStorage.getItem("sellwooData");
    let wooobj =JSON.parse(woo);
    let whe= localStorage.getItem("sellwheData");
    let wheobj =JSON.parse(whe);
    let baj= localStorage.getItem("sellbajData");
    let bajobj =JSON.parse(baj);
    let rag= localStorage.getItem("sellragData");
    ...
    function Del(val){
        ...
        if(val == 0)
            localStorage.setItem("sellcottData",obj);

        if(val == 1)
            localStorage.setItem("selljutData",obj);

        if(val == 2)
            localStorage.setItem("sellcoffData",obj);

        if(val == 3)
            localStorage.setItem("sellsteelData",obj);

        if(val == 4)
            localStorage.setItem("sellalumData",obj);

        if(val == 5)
            localStorage.setItem("sellcoppData",obj);

        if(val == 6)
            localStorage.setItem("sellwooData",obj);

        if(val == 7)
            localStorage.setItem("sellwheData",obj);

        if(val == 8)
            localStorage.setItem("sellbajData",obj);

        if(val == 9)
            localStorage.setItem("sellragData",obj);
    }
                    ...
                    val+1,
                        item.quantity>0 ? (
                            <div id='detailscart' key={item.itemName}>
                                ...
                                <h5><button  id="button-result" onClick={()=>{Del(val)}}>Delete</button></h5>
                                -------------------------------------
                            </div>
                        ) : ("") )
                )}
            </div>
            <button  id="button-result" onClick={savedetail}><Link id="sign" to='/homepage3'>Confirm Cart</Link></button>
