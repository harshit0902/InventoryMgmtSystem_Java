...
public class SellerInvoiceService {
    ...
    @Autowired
    public SellerInvoiceService(SellerInvoiceJpaRepository sellerInvoiceJpaRepository) {
        this.sellerInvoiceJpaRepository = sellerInvoiceJpaRepository;
    }

    public int insertIntoSellerInvoice(SellerInvoice special) {
        SellerInvoice result = mongoOperations.insert(special, "SellerInvoice");
    ...
    public List<SellerInvoice> findData(String email) {
        Query q2 = new Query();
        q2.addCriteria(Criteria.where("sellEmail").is(email));
        List<SellerInvoice> data = mongoOperations.find(q2, SellerInvoice.class);
